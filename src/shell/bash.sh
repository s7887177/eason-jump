# eason-jump shell integration for Bash
# Sourced from ~/.bashrc via 'j init'
...
# Install the cd hook via PROMPT_COMMAND
if [[ ! "${PROMPT_COMMAND:-}" == *"__eason_jump_cd_hook"* ]]; then
    PROMPT_COMMAND="__eason_jump_cd_hook${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
fi
